...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Window length L for NSH averaging (default 4)",
    )
    parser.add_argument(
        "--nsh-max-t",
        type=int,
        default=8,
        help="Max t offset to search for NSH (default 8)",
    )
    ...
    ff = FlightFeatures(
        ...
        nsh_window_L=int(args.nsh_window_L),
        nsh_max_t=int(args.nsh_max_t),
    )
    ...
    for fid in selected_flights:
        ...
        # -------------------------------------------------------------
        # Manual NSH score computation (non-vectorized)
        # J(t, L) = (1/L_eff) * sum_{k=0..L-1 and valid} min_s sl_s((t+k)+xi_s)
        # with xi_s equal to earliest hour at that TV, offset by min earliest across TVs
        # Then J = max_t J(t,L) for t in [1, nsh_max_t]
        def compute_manual_nsh_for_flight() -> float:
            L = int(args.nsh_window_L)
            max_t = int(args.nsh_max_t)
            hourly_matrix_local = getattr(evaluator, "last_hourly_occupancy_matrix", None)
            if hourly_matrix_local is None:
                return 0.0

            # Build hours_by_tv and earliest arrivals for this flight
            _occ_tv_set, hours_map = compute_footprint_and_hours_from_occupancy(
                flight_list, fid
            )

            # Collect per-TV earliest hours and row indices
            per_tv: List[Tuple[int, str, int]] = []  # (row_idx, tv_id, xi_raw)
            for tv_id in sorted(_occ_tv_set):
                row_idx = evaluator.tv_id_to_row_idx.get(tv_id)
                hours = sorted(hours_map.get(tv_id, set()))
                if row_idx is None or not hours:
                    continue
                xi_raw = int(hours[0])
                per_tv.append((int(row_idx), tv_id, xi_raw))

            if not per_tv:
                return 0.0

            anchor = min(xi for (_r, _tv, xi) in per_tv)
            per_tv_shifted: List[Tuple[int, str, int]] = [
                (r, tv, int(xi - anchor)) for (r, tv, xi) in per_tv
            ]

            # Helper to compute slack at tv/hour (if capacity known and in range)
            def slack_at(row_idx: int, tv_id: str, hour: int) -> float | None:
                if hour < 0 or hour >= hourly_matrix_local.shape[1]:
                    return None
                cap = evaluator.hourly_capacity_by_tv.get(tv_id, {}).get(int(hour))
                if cap is None:
                    return None
                return float(cap) - float(hourly_matrix_local[row_idx, int(hour)])
            ...
            for t in range(1, max_t + 1):
                ...
                for k in range(0, L):
                    tau = int(t + k)
                    # min over TVs at this tau
                    min_slack: float | None = None
                    for row_idx, tv_id, xi in per_tv_shifted:
                        s = slack_at(row_idx, tv_id, tau + xi)
                        if s is None:
                            continue
                        if (min_slack is None) or (s < min_slack):
                            min_slack = s
                    if min_slack is not None:
                        window_vals.append(float(min_slack))
                if window_vals:
                    jt = float(sum(window_vals) / float(len(window_vals)))
                else:
                    jt = 0.0
                if jt > best_val:
                    best_val = jt
            return 0.0 if not np.isfinite(best_val) else float(best_val)

        manual_nsh = compute_manual_nsh_for_flight()
        ff_nsh = float(ff.get(fid).nsh_score)
        status_nsh = "OK" if abs(manual_nsh - ff_nsh) <= 1e-6 else "MISMATCH"
        print(
            f"  - NSH score manual={manual_nsh:.6f} | features={ff_nsh:.6f} => {status_nsh}"
        )
