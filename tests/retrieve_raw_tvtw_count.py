...
def test_hotspot_flight_retrieval():
    ...
    try:
        ...
        print(
            f"   ✅ Loaded {flight_list.num_flights} flights with {flight_list.num_tvtws} TVTWs"
        )

        # Initialize evaluator
        print("3. Initializing NetworkEvaluator...")
        evaluator = NetworkEvaluator(traffic_volumes_gdf, flight_list)
        print("   ✅ NetworkEvaluator initialized")

        # Export raw per-TVTW counts CSV
        print("4. Exporting raw per-TVTW flight counts to output/tvtw_raw_counts.csv ...")
        evaluator.export_raw_tvtw_counts_csv("output/tvtw_raw_counts.csv")
        print("   ✅ Saved CSV with columns [traffic_volume_id, time_window, raw_count]")
