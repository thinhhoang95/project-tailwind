...
def process_all_pairs_and_get_representatives(df: pd.DataFrame, output_path: str) -> None:
    """
...
if __name__ == "__main__":
    # Define file paths for the analysis
    file_to_load = "output/city_pairs/grouped_flights_by_cpairs/EGLF.csv"
    output_file = f"output/city_pairs/representatives/{os.path.basename(file_to_load).replace('.csv', '_representatives.csv')}"


    console.print(f"[bold blue]🛫 Trajectory Grouping Analysis for All Pairs[/bold blue]")
    console.print(f"[blue]Data file: {file_to_load}[/blue]")
    console.print(f"[blue]Output will be saved to: {output_file}[/blue]")
    console.print()

    try:
        # Load the dataset
        console.print("[yellow]📁 Loading flight data...[/yellow]")
        flights_df = pd.read_csv(file_to_load)
        console.print(f"[green]✓ Successfully loaded {len(flights_df)} flight records[/green]")
        console.print()

        # Process all airport pairs to get representative trajectories and save to file
        process_all_pairs_and_get_representatives(flights_df, output_file)

    except FileNotFoundError:
        console.print(f"[bold red]❌ File Not Found[/bold red]")
        console.print(f"[red]The file '{file_to_load}' was not found.[/red]")
        console.print("[red]Please ensure the file path is correct and the script is run from the project root directory.[/red]")
