...
class AirspaceAPIWrapper:
    ...
    async def get_flow_extraction_parrhesia(
        self,
        traffic_volume_id: str,
        ref_time_str: str,
        *,
        ...
        """
        Assign community labels using parrhesia's flow extractor implementation.

        Mirrors the legacy endpoint behavior but computes clustering via
        `parrhesia.flows.flow_extractor.assign_communities_for_hotspot`.
        """
        self._ensure_evaluator_ready()
        ...
        # Compute hour from ref time (metadata only)
        try:
            ref_seconds = _hour * 3600 + _minute * 60 + _second
            _ = int(ref_seconds // 3600)
        except Exception:
            pass
        ...
        def _compute_assignments() -> Dict[str, int]:
            from parrhesia.flows.flow_extractor import (
                assign_communities_for_hotspot as parr_assign_communities_for_hotspot,
            )
            return parr_assign_communities_for_hotspot(
                self._flight_list,
                candidate_flight_ids,
                traffic_volume_id,
                threshold=float(threshold),
        ...
        try:
            ...
            print(
                f"Error computing parrhesia flow extraction for {traffic_volume_id} at {ref_time_str}: {e}"
            )
            raise e
