...
def main() -> None:
    ...
    # Precompute union of all flight ids for setwise comparisons
    all_flight_ids: set[str] = set()
    for _fid, _ids in flights_by_flow.items():
        all_flight_ids.update(_ids)

    # Accumulators
    setwise: Dict[int, Dict[str, float]] = {}
    ...
    for b in timebins:
        ...
        # 9b) Setwise features: each flow vs union of other flows' flights; accumulate across bins
        flow_ids = list(xG_map.keys())
        for i in range(len(flow_ids)):
            fi = int(flow_ids[i])
            # Against-set = all flights except those in fi
            others = list(all_flight_ids.difference(set(flights_by_flow.get(int(fi), []))))
            ctrl_i = ctrl_by_flow.get(fi) or hotspot_tv  # fallback to hotspot tv if missing

            feats = compare_flow_against_flight_set(
                anchor_tG=int(tG_b[fi]),
                anchor_tau=tau_map[fi],
                anchor_xG=xG_map[fi],
                v_tilde_anchor=float(vtilde_b[fi]),
                set_flight_ids=others,
                control_tv_for_set=str(ctrl_i),
            ...
            acc = setwise.get(int(fi))
            if acc is None:
                acc = {"overlap": 0.0, "orth": 0.0, "slack_corr": 0.0, "price_gap": 0.0}
                setwise[int(fi)] = acc
            acc["overlap"] += feats["overlap"]
            acc["orth"] += feats["orth"]
            acc["slack_corr"] += feats["slack_corr"]
            acc["price_gap"] += feats["price_gap"]
    ...
    # Optional: setwise summary
    if setwise:
        from rich.table import Table as _Table
        tbl = _Table(title="Setwise features (Flow vs union of other flights; summed across bins)")
        tbl.add_column("Flow", style="bold")
        for col in ["overlap", "orth", "slack_corr", "price_gap"]:
            tbl.add_column(col, justify="right")
        for fid, metrics in setwise.items():
            tbl.add_row(str(fid), *(f"{metrics[c]:.3f}" for c in ["overlap","orth","slack_corr","price_gap"]))
        console.print(tbl)
